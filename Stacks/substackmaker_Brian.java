 ...
 *
 *
 * BRIAN KIM EDITED  5/31/2019
 * ADDED MOD FUNCTIONALITY TO RANGE SELECTIONS
 * NO MOD WILL INCREMENT VALUES BY 1 TO CREATE STACK
 * MOD WILL INCREMENT VALUES BY MOD TO CREATE STACK
 *
...
public class substackmaker_Brian implements PlugInFilter {
    private ImagePlus       imp;
    private String          userInput, stackTitle, num, strA, strB;
    private int             rangeStart, rangeEnd, range, currSlice, count, mod; //BKIM ADDED MOD
    ...
    public void run(ImageProcessor ip) {
        ...
        try{
            ...
            int modx = userInput.indexOf(":"); //TO DETECT MOD INPUT
            if (modx != -1) {
                String rngStart = userInput.substring(0, idx1);
                String rngEnd = userInput.substring(idx1 + 1, modx);
                String modValue = userInput.substring(modx + 1);
                //IJ.log("start:" + rngStart + " end:" + rngEnd + " mod:" + modValue);
                obj = new Integer(rngStart);
                rangeStart = obj.intValue();
                obj = new Integer(rngEnd);
                rangeEnd = obj.intValue();
                obj = new Integer(modValue);
                mod = obj.intValue();
                range = rangeEnd - rangeStart + 1;
                stackRange(rangeStart, stackTitle, mod);
            } else {
                if (idx1 >= 1) {                                                    // input displayed in range
                    ...
                    rangeEnd = obj.intValue();
                    range = rangeEnd - rangeStart + 1;
                    stackRange(rangeStart, stackTitle, 1);
                } else {
    ...
    //BKIM, EDITED THIS METHOD TO ADD MOD FUNCTIONALITY, FUNCTIONS SAME WITHOUT THE MOD VALUE
    void stackRange(int currSlice, String stackTitle, int mod){                  // extract range of slices
        try{
            int width = imp.getWidth();
            int height = imp.getHeight();
            ImageStack stack = imp.getStack();
    ...
    void getInput(){
        ...
        gd.addMessage("Enter either range (e.g. 2-14) or a list (e.g. 7,9,25,27). You can use mod values (e.g. 5-500:10)");
